...
`),M=cr(C);M&&(f.has(M)?$.add(M):f.set(M,{id:M,source:C})),E+=N+1}let F=!1;for(let E of p){if(/^```/.test(E)){F=!F;continue}if(!F){let N=sr(E);N&&x.set(N.id,(x.get(N.id)||0)+1)}}return{definitions:f,duplicateDefinitionIds:$,referenceCounts:x}})(),{definitions:c,duplicateDefinitionIds:l,referenceCounts:u}=a;function d(f){let $=n[f]||"";return!$.trim()||/^```/.test($)||/^(#{1,6})\s+/.test($)||/^ {0,3}&gt;|^ {0,3}>/.test($)||/^ {0,3}(?:[-*_]\s*){3,}$/.test($)||/^:::/.test($)||!!Ne($)||f+1<n.length&&!!ar(n[f+1])}function g(f,$){let x=Ne(n[f]),p=/^\d/.test(x[2]),F=[],E=f,N=p?Number.parseInt(x[2],10):null;for(;E<n.length;){let S=Ne(n[E]);if(!S||S[1].length!==$||/^\d/.test(S[2])!==p)break;let v={content:[S[3]],children:[]};for(E+=1;E<n.length;){let A=Ne(n[E]);if(A&&A[1].length>$){let D=g(E,A[1].length);v.children.push(D.html),E=D.index;continue}if(!n[E].trim()){E+=1;let D=E<n.length?Ne(n[E]):null;if(E>=n.length||!D||D[1].length<=$)break;continue}if(/^\s+/.test(n[E])&&!Ne(n[E])){v.content.push(n[E].trim()),E+=1;continue}break}F.push(v)}let C=p?"ol":"ul",M=p&&N!==1?` start="${N}"`:"",R=F.map(S=>{let v=!p&&S.content.length===1&&S.content[0].match(/^\[([ xX])\]\s+(.*)$/),A=v?`<input type="checkbox" disabled${v[1].toLowerCase()==="x"?" checked":""}> ${Ee(v[2])}`:Ee(S.content.join(" "));return`<li${v?' class="docdiagram-task-list-item"':""}>${A}${S.children.join("")}</li>`}).join("");return{html:`<${C}${M}>${R}</${C}>`,index:E}}function y(f,$){let x=Ot(n[f]),p=x?Or(n,f,$):-1;if(!x||p===-1)return null;let{name:F,attributes:E}=x,N={section:["title","palette","fill","stroke","text"],panel:["title","palette","fill","stroke","text"],callout:["kind","title","palette","fill","stroke","text"],grid:["columns"],stack:[]};if(Object.keys(E).some(v=>!N[F].includes(v)))return null;if(F==="grid"){let v=Xt[E.columns];if(!v)return null;let A=[],D=f+1;for(;D<p;){if(!n[D].trim()){D+=1;continue}let W=Ot(n[D]);if(!W||!["panel","callout","stack"].includes(W.name))return null;let U=y(D,p);if(!U)return null;A.push(`<div class="docdiagram-grid-item">${U.html}</div>`),D=U.next}return{html:`<div class="docdiagram-grid" style="--docdiagram-grid-columns:${v}">${A.join("")}</div>`,next:p+1}}if(F==="stack")return Object.keys(E).length?null:{html:`<div class="docdiagram-stack">${h(f+1,p)}</div>`,next:p+1};let C=Gr(E,i,s);if(C===null||F==="callout"&&E.kind!==void 0&&!_t.includes(E.kind))return null;let M=E.title?`<div class="docdiagram-component-title">${Ee(E.title)}</div>`:"",R=h(f+1,p),S=`docdiagram-component${F==="callout"?"":` docdiagram-${F}`}${C?" docdiagram-component-styled":""}`;if(F==="callout"){let v=E.kind||"info";return{html:`<aside class="${S} docdiagram-callout docdiagram-callout-${v}"${C?` style="${C}"`:""} aria-label="${w(E.title||v)} callout"><div class="docdiagram-callout-kind">${w(v)}</div>${M}${R}</aside>`,next:p+1}}return{html:`<section class="${S}"${C?` style="${C}"`:""}>${M}${R}</section>`,next:p+1}}function h(f=0,$=n.length){let x=[],p=f;for(;p<$;){let F=n[p];if(!F.trim()){p+=1;continue}if(/^:::/.test(F)){let S=sr(F);if(S){let A=c.get(S.id),D=u.get(S.id)||0;A?l.has(S.id)?x.push(`<section class="docdiagram-error"><strong>Diagram "${w(S.id)}" has multiple definitions.</strong></section>`):D>1?x.push(`<section class="docdiagram-error"><strong>Diagram "${w(S.id)}" is referenced more than once.</strong></section>`):(x.push(o(A.source,e.diagramIndex)),e.diagramIndex+=1):x.push(`<section class="docdiagram-error"><strong>Diagram "${w(S.id)}" could not be found.</strong></section>`),p+=1;continue}let v=y(p,$);v?(x.push(v.html),p=v.next):(x.push(`<pre class="docdiagram-literal-source"><code>${w(F)}</code></pre>`),p+=1);continue}let E=F.match(/^```([\w-]*)\s*$/);if(E){let S=n.slice(p+1,$).findIndex(D=>/^```\s*$/.test(D));if(S===-1){x.push('<section class="docdiagram-error"><strong>Unclosed code block.</strong></section>');break}let v=p+S+1,A=n.slice(p+1,v).join(`
`);if(E[1]==="diagram"){let D=cr(A);D&&l.has(D)?x.push(`<section class="docdiagram-error"><strong>Diagram "${w(D)}" has multiple definitions.</strong></section>`):(!D||!u.has(D))&&(x.push(o(A,e.diagramIndex)),e.diagramIndex+=1)}else{let D=E[1]?` class="language-${w(E[1])}"`:"";x.push(`<pre><code${D}>${w(A)}</code></pre>`)}p=v+1;continue}let N=F.match(/^(#{1,6})\s+(.+?)\s*#*\s*$/);if(N){x.push(`<h${N[1].length}>${Ee(N[2])}</h${N[1].length}>`),p+=1;continue}if(/^ {0,3}(?:[-*_]\s*){3,}$/.test(F)){x.push("<hr>"),p+=1;continue}if(/^ {0,3}>/.test(F)){let S=[];for(;p<$&&/^ {0,3}>/.test(n[p]);)S.push(n[p].replace(/^ {0,3}> ?/,"")),p+=1;x.push(`<blockquote>${Vt(S.join(`
`),e,{...t,diagramReferenceRegistry:a})}</blockquote>`);continue}let C=Ne(F);if(C){let S=g(p,C[1].length);x.push(S.html),p=S.index;continue}let M=p+1<$?ar(n[p+1]):null;if(M){let S=jt(F),v=[];for(p+=2;p<$&&n[p].includes("|")&&n[p].trim();)v.push(jt(n[p])),p+=1;let A=(D,W)=>W.map((U,ue)=>`<${D}${M[ue]?` style="text-align:${M[ue]}"`:""}>${Ee(U||"")}</${D}>`).join("");x.push(`<table><thead><tr>${A("th",S)}</tr></thead><tbody>${v.map(D=>`<tr>${A("td",D)}</tr>`).join("")}</tbody></table>`);continue}let R=[F.trim()];for(p+=1;p<$&&!d(p);)R.push(n[p].trim()),p+=1;x.push(`<p>${Ee(R.join(" "))}</p>`)}return x.join("")}return h()}function Gt(r,e,t){let n=e!=="none",o=e==="flowchart";return['<div class="docdiagram-diagram-toolbar" role="toolbar" aria-label="Diagram controls">',`<button type="button" class="docdiagram-icon-button docdiagram-zoom-in" data-diagram-index="${r}" aria-label="Zoom in" title="Zoom in">+</button>`,`<button type="button" class="docdiagram-icon-button docdiagram-zoom-out" data-diagram-index="${r}" aria-label="Zoom out" title="Zoom out">\u2212</button>`,`<button type="button" class="docdiagram-icon-button docdiagram-fit" data-diagram-index="${r}" aria-label="Zoom to fit" title="Zoom to fit">\u22A1</button>`,n?t.editingDiagramIndex===r?`<button type="button" class="docdiagram-icon-button docdiagram-done-editing" aria-label="Done editing" title="Done editing">\u2713</button><button type="button" class="docdiagram-icon-button docdiagram-cancel-editing" aria-label="Cancel editing and discard changes" title="Cancel editing and discard changes">\xD7</button>${o?`<button type="button" class="docdiagram-icon-button docdiagram-create-node" data-diagram-index="${r}" aria-label="New node" title="New node">+</button>`:""}`:t.editingDiagramIndex===null?'<button type="button" class="docdiagram-icon-button docdiagram-start-editing" aria-label="Edit diagram" title="Edit diagram">\u270E</button>':"":"","</div>"].join("")}function dr(r,e,t,n){let{selectedNode:o,selectedEdge:i,editingNode:s,editingEdge:a,connectionDrag:c,diagramZooms:l}=t,u=t.editingDiagramIndex===e,d=O(r),g=new Map(d.map(S=>[S.node.id,S])),y=16,h=[],f=[],x=ie[t.documentColorScheme]?.[t.documentTheme==="dark"?"dark":"light"],p=x?Object.entries(x).filter(([,S])=>S.gradient).map(([S,v])=>`<linearGradient id="docdiagram-${t.documentColorScheme}-${e}-${S}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="${w(v.gradient||v.fill)}"/><stop offset="1" stop-color="${w(v.fill)}"/></linearGradient>`).join(""):"",F=r.edges.map((S,v)=>{let A=g.get(S.source),D=g.get(S.target);if(!A||!D)return"";let W=A.node,U=D.node,ue=fe(W,A.position.x,A.position.y,Number(W.size?.width)||190,Number(W.size?.height)||80),re=fe(U,D.position.x,D.position.y,Number(U.size?.width)||190,Number(U.size?.height)||80),Y=S.sourceAnchor||"right",j=S.targetAnchor||"left",z=ue.anchors[Y],K=re.anchors[j],oe=S.route||"orthogonal",B=De(z,K,Y,j,oe),ne=B.midpoint.x,me=B.midpoint.y-10,ge=Ie(r,S,t.documentTheme),Te=i?.diagramIndex===e&&i.edgeIndex===v,Nt=Te&&a?.diagramIndex===e&&a.edgeIndex===v,We=(Number(ge.strokeWidth)||2)+(Te?2:0),nt=220,it=72,Ue=S.label?ce(S.label):[],b=Ue.length*y,k=me-b/2+y*.72,P=Xe(S,"start"),T=Xe(S,"end"),I=`docdiagram-marker-${e}-${v}-start`,Z=`docdiagram-marker-${e}-${v}-end`;P!=="none"&&h.push(ot(I,P,"start",ge.stroke||"",We)),T!=="none"&&h.push(ot(Z,T,"end",ge.stroke||"",We)),Te&&u&&f.push(`<circle class="docdiagram-edge-endpoint" data-diagram-index="${e}" data-edge-index="${v}" data-endpoint="source" cx="${z.x}" cy="${z.y}" r="7"/>`,`<circle class="docdiagram-edge-endpoint" data-diagram-index="${e}" data-edge-index="${v}" data-endpoint="target" cx="${K.x}" cy="${K.y}" r="7"/>`);let H=[P!=="none"?` marker-start="url(#${I})"`:"",T!=="none"?` marker-end="url(#${Z})"`:""].join("");return[`<g class="docdiagram-edge-group${Te?" docdiagram-edge-selected":""}" data-diagram-index="${e}" data-edge-index="${v}">`,`<path class="docdiagram-edge-hit" d="${B.hitPath}" fill="none" stroke="transparent" stroke-width="16"/>`,`<path class="docdiagram-edge" d="${B.path}"${H} stroke="${w(ge.stroke||"")}" stroke-width="${We}"/>`,Nt?`<foreignObject class="docdiagram-inline-editor-host" x="${ne-nt/2}" y="${me-it/2}" width="${nt}" height="${it}"><textarea class="docdiagram-inline-editor docdiagram-inline-editor-edge" aria-label="Edit edge label. Press Enter for a new line. Press Control or Command plus Enter to save. Press Escape to cancel.">${w(S.label||"")}</textarea></foreignObject>`:Ue.length?ee(ne,k,Ue,y,"docdiagram-edge-label",ge.text||""):"","</g>"].join("")}).join(""),E=d.map(({node:S,position:v})=>{let A=v.x,D=v.y,W=Number(S.size?.width)||190,U=Number(S.size?.height)||80,ue=xe(r,S,t.documentTheme,t.documentColorScheme),re=S.palette,Y=re&&x?.[re]?.gradient?{...ue,fill:`url(#docdiagram-${t.documentColorScheme}-${e}-${re})`}:ue,j=o?.diagramIndex===e&&o.nodeId===S.id,z=j&&s?.diagramIndex===e&&s.nodeId===S.id,K=(Number(Y.strokeWidth)||2)+(j?2:0),oe=fe(S,A,D,W,U),B=Ve(oe.textBounds,S);return[`<g class="docdiagram-node${j?" docdiagram-node-selected":""}" data-diagram-index="${e}" data-node-id="${w(S.id)}">`,Ge(oe,Y,K),z?`<foreignObject class="docdiagram-inline-editor-host" x="${oe.textBounds.x}" y="${oe.textBounds.y}" width="${oe.textBounds.width}" height="${oe.textBounds.height}"><textarea class="docdiagram-inline-editor docdiagram-inline-editor-node" aria-label="Edit node label. Press Enter for a new line. Press Control or Command plus Enter to save. Press Escape to cancel.">${w(S.label)}</textarea></foreignObject>`:ee(B.centerX,B.labelStartY,B.labelLines,B.labelLineHeight,"docdiagram-node-label",Y.text||"",B.textAnchor),!z&&B.subtitleLines.length?ee(B.centerX,B.subtitleStartY,B.subtitleLines,B.subtitleLineHeight,"docdiagram-node-subtitle",Y.text||"",B.textAnchor):"",j&&u&&!z?[["top-left",A-7,D-7],["top-right",A+W-7,D-7],["bottom-left",A-7,D+U-7],["bottom-right",A+W-7,D+U-7]].map(([ne,me,ge])=>`<rect class="docdiagram-resize-handle" data-resize-corner="${ne}" x="${me}" y="${ge}" width="14" height="14" rx="3"/>`).join(""):"",j&&u&&!z?J.map(ne=>{let me=oe.anchors[ne];return`<circle class="docdiagram-connection-port" data-anchor="${ne}" cx="${me.x}" cy="${me.y}" r="7" aria-label="${ne} connection port"/>`}).join(""):"","</g>"].join("")}).join(""),N=Number(r.canvas.width)||1e3,C=Number(r.canvas.height)||560,M=t.diagramViewportHeights.get(e),R=M?` style="box-sizing: border-box; height: ${M}px"`:"";return[`<figure class="docdiagram" data-diagram-index="${e}" data-diagram-type="flowchart" data-editing="${u}"${R}>`,n(e,"flowchart",t),`<svg viewBox="0 0 ${N} ${C}" role="img" aria-label="Architecture diagram" data-diagram-index="${e}" style="width: ${l.get(e)||100}%">`,`<defs>${p}${h.join("")}</defs>`,E,F,c?.diagramIndex===e?`<path class="docdiagram-connection-preview${c.invalid?" docdiagram-connection-invalid":""}" d="${De(c.start,c.current,c.sourceAnchor,c.targetAnchor||c.sourceAnchor,"straight").path}"/>`:"",f.join(""),"</svg>","</figure>"].join("")}function lr(r,e,t,n){let o=ve(r,t.documentTheme),i=Number(r.canvas?.width)||1e3,s=Number(r.canvas?.height)||560,a=r.participants||[],c=r.messages||[],l=r.activations||[],u=r.notes||[],d=r.groups||[],g=90,y=90,h=28,f=Number(r.canvas?.participantSize?.width)||180,$=Number(r.canvas?.participantSize?.height)||42,x=Number(r.canvas?.participantSpacing)||220,p=16,F=74+Math.max(0,...a.filter(b=>b.kind==="actor").map(b=>ce(b.label||"").length-1))*p,E=48,N=18,C=56,M=t.diagramViewportHeights.get(e),R=M?` style="box-sizing: border-box; height: ${M}px"`:"",S=`docdiagram-sequence-arrow-${e}`,v=h+F+12,A=a[0],D=a[a.length-1],W=Number(A?.size?.width)||f,U=Number(D?.size?.width)||f,ue=a.length>1?W/2+x*(a.length-1)+U/2:f+g+y,re=Math.max(i,ue,g+y),Y=new Map;a.forEach((b,k)=>{Y.set(b.id,a.length===1?re/2:W/2+x*k)});let j=v+40,z=c.map((b,k)=>({...b,index:k,y:j+k*C})),K=u.map(b=>{let k=ce(b.label||""),P=Math.max(E,k.length*16+22,Number(b.size?.height)||0),I=((b.after?z[Number(b.after)-1]:null)?.y||v)+N,Z=Y.get(b.at||"")||re/2,H=Math.max(160,Number(b.size?.width)||0),he=Math.min(re-H/2-24,Math.max(H/2+24,Z));return{...b,lines:k,x:he-H/2,y:I,width:H,height:P}}),oe=d.map(b=>z[b.to-1]?.y+34||j),B=Math.max(v+140,K.length?K[K.length-1].y+K[K.length-1].height:0,z.length?z[z.length-1].y+44:j,...oe),ne=Math.max(s,B+56),me=ne-36,ge=l.map((b,k)=>({participantId:b.participant,depth:l.slice(0,k).filter(P=>P.participant===b.participant&&P.from<=b.from&&P.to>=b.from).length,startY:(z[b.from-1]?.y||j)-10,endY:(z[b.to-1]?.y||j)+18})),Te=a.map(b=>{let k=Y.get(b.id)||0,P=ce(b.label||""),T=Be(r,b,t.documentTheme,t.documentColorScheme),I=Number(b.size?.width)||f,Z=Number(b.size?.height)||$;if(b.kind==="actor"){let H=h+10,he=H+18,qe=he+18;return[`<g class="docdiagram-sequence-participant docdiagram-sequence-actor" data-diagram-index="${e}" data-participant-id="${w(b.id)}">`,`<circle cx="${k}" cy="${H}" r="8" fill="none" stroke="${w(T.stroke||"")}" stroke-width="${Number(T.strokeWidth)||2}"/>`,`<path d="M ${k} ${H+8} V ${qe} M ${k-14} ${he} H ${k+14} M ${k} ${qe} L ${k-12} ${qe+18} M ${k} ${qe} L ${k+12} ${qe+18}" fill="none" stroke="${w(T.stroke||"")}" stroke-width="${Number(T.strokeWidth)||2}" stroke-linecap="round" stroke-linejoin="round"/>`,ee(k,h+F-4-(P.length-1)*p,P,p,"docdiagram-node-label",T.text||""),"</g>"].join("")}return[`<g class="docdiagram-sequence-participant" data-diagram-index="${e}" data-participant-id="${w(b.id)}">`,`<rect x="${k-I/2}" y="${h}" width="${I}" height="${Z}" rx="12" fill="${w(T.fill||"")}" stroke="${w(T.stroke||"")}" stroke-width="${Number(T.strokeWidth)||2}"/>`,ee(k,h+Z/2+6-(P.length-1)*p/2,P,p,"docdiagram-node-label",T.text||""),"</g>"].join("")}).join(""),Nt=a.map(b=>{let k=Y.get(b.id)||0;return`<path class="docdiagram-sequence-lifeline" d="M ${k} ${v} L ${k} ${me}" fill="none" stroke="${w(o.edge.stroke)}" stroke-width="2" stroke-dasharray="8 6"/>`}).join(""),We=d.map(b=>{let k=(z[b.from-1]?.y||j)-24,P=(z[b.to-1]?.y||j)+30,T=Math.min(220,Math.max(110,String(b.label).length*8+28));return['<g class="docdiagram-sequence-group">',`<rect x="42" y="${k}" width="${re-84}" height="${P-k}" rx="12" fill="none" stroke="${w(o.edge.stroke)}" stroke-width="2" stroke-dasharray="10 6" opacity="0.8"/>`,`<rect x="54" y="${k-16}" width="${T}" height="24" rx="6" fill="${w(o.node.fill)}" stroke="${w(o.edge.stroke)}" stroke-width="1.5"/>`,`<text x="${54+T/2}" y="${k+1}" text-anchor="middle" class="docdiagram-edge-label" fill="${w(o.edge.text)}">${w(b.label||"")}</text>`,"</g>"].join("")}).join(""),nt=K.map((b,k)=>{let T=b.y+18,I=Be(r,b,t.documentTheme,t.documentColorScheme);return[`<g class="docdiagram-sequence-note" data-diagram-index="${e}" data-note-index="${k}">`,`<rect x="${b.x}" y="${b.y}" width="${b.width}" height="${b.height}" rx="10" fill="${w(I.fill||"")}" stroke="${w(I.stroke||"")}" stroke-width="${Number(I.strokeWidth)||2}"/>`,ee(b.x+b.width/2,T,b.lines,16,"docdiagram-node-subtitle",I.text||""),"</g>"].join("")}).join(""),it=ge.map(b=>{let k=Y.get(b.participantId)||0,P=b.depth*7,T=12,I=Math.max(20,b.endY-b.startY),Z=a.find(he=>he.id===b.participantId),H=Z?Be(r,Z,t.documentTheme,t.documentColorScheme):o.node;return`<rect class="docdiagram-sequence-activation" x="${k-T/2+P}" y="${b.startY}" width="${T}" height="${I}" rx="4" fill="${w(H.fill||"")}" stroke="${w(H.stroke||"")}" stroke-width="${Number(H.strokeWidth)||2}"/>`}).join(""),Ue=z.map(b=>{let k=Y.get(b.from)||0,P=Y.get(b.to)||0,T=b.style==="dashed",I=ce(b.label||""),Z=I.length*15,H=b.y-12-Z/2+11,he=` marker-end="url(#${S})"`;return b.from===b.to?[`<g class="docdiagram-sequence-message" data-diagram-index="${e}" data-message-index="${b.index}">`,`<path d="M ${k} ${b.y} L ${k+48} ${b.y} L ${k+48} ${b.y+28} L ${k} ${b.y+28}" fill="none" stroke="${w(o.edge.stroke)}" stroke-width="2"${he}${T?' stroke-dasharray="8 5"':""}/>`,ee(k+48/2,H,I,15,"docdiagram-edge-label",o.edge.text),"</g>"].join(""):[`<g class="docdiagram-sequence-message" data-diagram-index="${e}" data-message-index="${b.index}">`,`<path d="M ${k} ${b.y} L ${P} ${b.y}" fill="none" stroke="${w(o.edge.stroke)}" stroke-width="2"${he}${T?' stroke-dasharray="8 5"':""}/>`,ee((k+P)/2,H,I,15,"docdiagram-edge-label",o.edge.text),"</g>"].join("")}).join("");return[`<figure class="docdiagram" data-diagram-index="${e}" data-diagram-type="sequence" data-editing="${t.editingDiagramIndex===e}"${R}>`,n(e,"sequence",t),`<svg viewBox="0 0 ${re} ${ne}" role="img" aria-label="Sequence diagram" data-diagram-index="${e}" style="width: ${t.diagramZooms.get(e)||100}%">`,`<defs>${ot(S,"arrow","end",o.edge.stroke,2)}</defs>`,We,Te,Nt,it,nt,Ue,"</svg>","</figure>"].join("")}function ur(r,e,t){try{let n=Se(r,t.colourScheme);return t.onDiagram(e,n),n.type==="sequence"?lr(n,e,t.state,Gt):dr(n,e,t.state,Gt)}catch(n){let o=n instanceof Error?n.message:String(n);return`<section class="docdiagram-error"><strong>Diagram could not be rendered.</strong><br>${w(o)}</section>`}}function mr(){if(document.querySelector("style[data-docdiagram-runtime-styles]"))return;let r=document.createElement("style");r.dataset.docdiagramRuntimeStyles="true",r.textContent=`
    html,
    body {
      margin: 0;
      min-height: 100%;
